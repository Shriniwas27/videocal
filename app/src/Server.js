...
// API endpoint to download transcript file for a meeting (room)
app.get('/api/transcript/:roomId', (req, res) => {
    const roomId = req.params.roomId;
    const fileName = path.join(transcriptDir, `transcript_${sanitizeFilename(roomId)}.txt`);
    if (!fs.existsSync(fileName)) {
        return res.status(404).json({ error: 'Transcript not found' });
    }
    res.download(fileName, `transcript_${roomId}.txt`);
});
    ...
    */
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('cmd', async (dataObject) => {
            ...
            // Append transcript to file if type is transcript
            if (data.type === 'transcript' && data.text_data && data.room_id) {
                try {
                    const fileName = path.join(transcriptDir, `transcript_${sanitizeFilename(data.room_id)}.txt`);
                    const line = `[${new Date(data.time_stamp).toISOString()}] ${data.peer_name}: ${data.text_data}\n`;
                    fs.appendFile(fileName, line, (err) => {
                        if (err) log.error('Failed to append transcript:', err);
                    });
                } catch (err) {
                    log.error('Transcript file error:', err);
                }
            }
